...
vp::IoReqStatus LightRedmule::send_tcdm_req()
{
    ...
    // Single-word size (typically 4 bytes)
    uint32_t word_size = this->tcdm_bank_width;
    
    // Send multiple single-word requests
    for (uint32_t offset = 0; offset < total_size; offset += word_size) {
        uint32_t current_size = std::min(word_size, total_size - offset);
        
        // Create a temporary request for single word
        // Correct parameter order for req_new method
        ...
        // If any request fails, return that status
        if (status != vp::IO_REQ_OK) {
            overall_status = status;
            break;
        }
        
        // Copy the latency from the last request to the original request
...
void matmul_fp32(float * z, float * y, float * x, float * w, 
    uint16_t m_size, uint16_t n_size, uint16_t k_size) {
// 调试头和基本信息
printf("========== FP32矩阵乘法调试开始 ==========\n");
printf("矩阵维度: m_size=%d, n_size=%d, k_size=%d\n", m_size, n_size, k_size);
printf("缓冲区指针: z=%p, y=%p, x=%p, w=%p\n", z, y, x, w);

// 检查指针是否为NULL
if (z == NULL || y == NULL || x == NULL || w == NULL) {
printf("错误: matmul_fp32中检测到NULL指针!\n");
return;
}

// 验证输入维度不为零
if (m_size == 0 || n_size == 0 || k_size == 0) {
printf("错误: matmul_fp32中检测到零维度: m=%d, n=%d, k=%d\n", m_size, n_size, k_size);
return;
}

// 检查不合理的维度，可能是内存错误
if (m_size > 1000 || n_size > 1000 || k_size > 1000) {
printf("警告: matmul_fp32中的矩阵维度非常大: m=%d, n=%d, k=%d\n", m_size, n_size, k_size);
}
...
// 检查缓冲区对齐和内存范围
uintptr_t x_alignment = (uintptr_t)x % sizeof(float);
uintptr_t w_alignment = (uintptr_t)w % sizeof(float);
uintptr_t y_alignment = (uintptr_t)y % sizeof(float);
uintptr_t z_alignment = (uintptr_t)z % sizeof(float);

if (x_alignment != 0 || w_alignment != 0 || y_alignment != 0 || z_alignment != 0) {
printf("警告: 检测到缓冲区对齐问题!\n");
printf("  x对齐: %lu\n", x_alignment);
printf("  w对齐: %lu\n", w_alignment);
printf("  y对齐: %lu\n", y_alignment);
printf("  z对齐: %lu\n", z_alignment);
...
// 打印缓冲区范围
printf("\n缓冲区范围:\n");
printf("  X: %p - %p (大小: %lu字节)\n", 
x, x + (m_size * n_size - 1), (unsigned long)(m_size * n_size * sizeof(float)));
printf("  W: %p - %p (大小: %lu字节)\n", 
w, w + (n_size * k_size - 1), (unsigned long)(n_size * k_size * sizeof(float)));
printf("  Y: %p - %p (大小: %lu字节)\n", 
y, y + (m_size * k_size - 1), (unsigned long)(m_size * k_size * sizeof(float)));
printf("  Z: %p - %p (大小: %lu字节)\n", 
z, z + (m_size * k_size - 1), (unsigned long)(m_size * k_size * sizeof(float)));

// 检查缓冲区重叠情况
if ((z <= x && x < z + m_size * k_size) || 
(x <= z && z < x + m_size * n_size))
printf("警告: Z和X缓冲区重叠!\n");
if ((z <= w && w < z + m_size * k_size) || 
(w <= z && z < w + n_size * k_size))
printf("警告: Z和W缓冲区重叠!\n");
if (z != y && ((z <= y && y < z + m_size * k_size) || 
      (y <= z && z < y + m_size * k_size)))
printf("警告: Z和Y缓冲区重叠(但不相等)!\n");
else if (z == y)
printf("注意: Z和Y是同一个缓冲区 - 进行就地累加\n");

// 验证k_size参数
printf("\nk_size检查: 应为%d，缓冲区实际宽度可能为%d (CE_WIDTH*(CE_PIPE+1))\n", 
k_size, (k_size <= 12 ? k_size : 12));

// 创建Y的本地副本用于初始化
printf("\n创建Y的本地副本用于初始化...\n");
float* y_copy = NULL;
if (m_size * k_size > 0 && m_size * k_size < 1000000)
y_copy = (float*)malloc(m_size * k_size * sizeof(float));

if (y_copy == NULL) {
printf("警告: 无法为Y副本分配内存或大小不合理! 直接使用原始Y...\n");
printf("直接初始化Z矩阵...\n");
for (int i = 0; i < m_size; i++) {
for (int j = 0; j < k_size; j++) {
   int idx = i * k_size + j;
   if (isnan(y[idx]) || isinf(y[idx]) || fabsf(y[idx]) > 1e20) {
       printf("警告: Y[%d]可疑值: %f，替换为0\n", idx, y[idx]);
       z[idx] = 0.0f;
   } else {
       z[idx] = y[idx];
   }
...
y_copy[i] = y[i];
if (isnan(y_copy[i]) || isinf(y_copy[i]) || fabsf(y_copy[i]) > 1e20) {
   printf("警告: Y[%d]可疑值: %f\n", i, y_copy[i]);
   y_copy[i] = 0.0f;
}
}
...
for (int i = 0; i < m_size; i++) {
for (int j = 0; j < k_size; j++)
   z[i * k_size + j] = y_copy[i * k_size + j];
}
free(y_copy);
y_copy = NULL;
}

// 打印初始化后Z矩阵的前几个元素（这里仍然只打印部分用于预览）
printf("初始化后的Z(前4个元素):");
for (int i = 0; i < ((m_size * k_size < 4) ? m_size * k_size : 4); i++)
printf(" %.6f", z[i]);
printf("\n");
...
float debug_sum = z[0];
for (int k = 0; k < n_size && k < 10; k++) {
float x_val = x[0 * n_size + k];
float w_val = w[k * k_size + 0];
float product = x_val * w_val;
debug_sum += product;
printf("  + x[0][%d] * w[%d][0] = %.6f * %.6f = %.6f (运行总和: %.6f)\n", 
      k, k, x_val, w_val, product, debug_sum);
}
if (n_size > 10)
printf("  (省略剩余%d项...)\n", n_size - 10);
printf("  预期Z[0][0]最终值 ≈ %.6f\n", debug_sum);
}
...
for (int j = 0; j < k_size; j++) {
for (int k = 0; k < n_size; k++) {
   float x_val = x[i * n_size + k];
   float w_val = w[k * k_size + j];
   float product = x_val * w_val;
   if (isnan(product) || isinf(product)) {
       printf("警告: 在 i=%d, j=%d, k=%d 处计算产生非法值: %.6f * %.6f\n", 
              i, j, k, x_val, w_val);
       continue;
   }
   z[i * k_size + j] += product;
   if (fabsf(z[i * k_size + j]) > 1e20)
       printf("警告: Z[%d][%d]值变得极大: %.6f\n", i, j, z[i * k_size + j]);
}
}
if (m_size > 10 && i % (m_size / 10) == 0)
printf("  %d%%完成...\n", (i * 100) / m_size);
}
