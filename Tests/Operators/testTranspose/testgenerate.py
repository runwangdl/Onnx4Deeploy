...
def generate_transpose_onnx_and_data(save_path=None):
    """ Generate ONNX model for Transpose operator based on config, with optional save path """
    ...
    model_config = config["transpose"]
        
    input_shape = tuple(model_config["input_shape"])  # [1, 16, 16, 32]
    output_shape = tuple(model_config["output_shape"])  # [1, 32, 16, 16]
    perm = tuple(model_config["perm"])  # [0, 3, 1, 2]
...
if __name__ == "__main__":
    ...
    generate_transpose_onnx_and_data(save_path)