...
def generate_matmul_onnx_and_data(save_path=None):
    """ Generate ONNX model for MatMul operator based on config, with optional save path """
    ...
    if len(input_a_shape) != 2 or len(input_b_shape) != 2:
        raise ValueError("Input shapes must be 2D matrices")
    
    if input_a_shape[1] != input_b_shape[0]:
        raise ValueError(f"Incompatible matrix dimensions for multiplication: {input_a_shape} and {input_b_shape}")
    
    # Calculate output shape
    output_shape = (input_a_shape[0], input_b_shape[1])
    ...
    input_a = np.random.randn(*input_a_shape).astype(np.float32)
    input_b = np.random.randn(*input_b_shape).astype(np.float32)
    ...
    matmul_node = helper.make_node("MatMul", inputs=["input_a", "input_b"], outputs=["output"], name="matmul_node")
    graph_def = helper.make_graph([matmul_node], "matmul_graph", [input_tensor_a, input_tensor_b], [output_tensor])
...
if __name__ == "__main__":
    ...
    generate_matmul_onnx_and_data(save_path)